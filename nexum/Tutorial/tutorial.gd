...
func _ready() -> void:
	...
	%Bottle.hide()
	%Apple.hide()
	label.text = "Bienvenido al entrenamiento. Pulsa ENTER para empezar."
...
func update_instruction() -> void:
	match current_step:
		...
		Steps.SHIELD:
			%Bottle.show()
			label.text = "Te iran apareciendo objetos, para que puedas aumentar tu nivel de escudo, acercate a ellos para poder obtenerlo."
		Steps.ATTACK:
			%Zombie.show()
		...
		Steps.OBJECTS:
			...
			if %Player.current_health < 100:
				%Zombie.hide()
			label.text = "Te iran apareciendo objetos, para que puedas aumentar tu nivel de vida, acercate a ellos para poder obtenerlo."
		Steps.FINISHED:
